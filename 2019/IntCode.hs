...
run :: State -> State
run (Initial ops input) = exec ops input Nothing 0 0
run (WaitingForInput ops index rBase input@(_ : _) _) =
  exec ops input Nothing rBase index
run state = state
...
exec ops input output rBase index = case opCode of
  ...
  4 -> exec ops input (Just $ op' 1) rBase $ index + 2
  ...
  99 -> Halted $ fromMaybe (findWithDefault 0 0 ops) output
